...
This writes deterministic sample signals to `.evofork/demo-seed.json`. The file
is local run state and is ignored by git.
...
该命令会把确定性的示例 signals 写入 `.evofork/demo-seed.json`。这个文件属于本地运行状态，已被 git 忽略。
